  ...
  <div class="h-full flex flex-col p-6 space-y-6 overflow-y-auto" @dragover.prevent @drop.prevent="handleFileDrop">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold tracking-tight text-white flex items-center space-x-3">
          <span>{{ t.apps?.title || 'Devices' }}</span>
          <span
    ...
    <div v-else class="p-8 rounded-2xl bg-[#171A21]/60 border border-white/[0.08] backdrop-blur-xl text-center flex flex-col items-center justify-center space-y-4">
      ...
      <div class="max-w-md">
        ...
        <p class="text-sm text-[#8E8E93] mt-1 leading-relaxed">{{ t.apps.noDeviceDesc }}</p>
      </div>
      <div class="pt-2 text-left max-w-lg w-full bg-white/[0.03] border border-white/[0.06] rounded-xl p-4 space-y-2 text-xs text-[#8E8E93]">
        <div class="font-medium text-white flex items-center space-x-1.5">
          <svg class="w-4 h-4 text-[#0A84FF]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
      ...
      <div class="space-y-2">
        <div v-for="task in deviceStore.installTasks" :key="task.id" class="glass-card p-4 rounded-2xl border border-white/[0.05] hover:border-white/10 transition-all flex flex-col space-y-3">
          <div class="flex items-center justify-between">
            ...
            <button @click="deviceStore.removeInstallTask(task.id)" v-if="task.phase === 'Complete' || task.phase === 'Failed'" class="p-2 rounded-xl hover:bg-white/10 text-[#7D8592] transition">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
          </div>
          <div class="space-y-1.5">
            <div class="w-full bg-white/5 rounded-full h-1.5 overflow-hidden">
              <div class="h-full rounded-full transition-all duration-500" :class="task.phase === 'Failed' ? 'bg-red-500' : task.phase === 'Complete' ? 'bg-[#30D158]' : 'bg-[#0A84FF]'" :style="{ width: `${task.percent}%` }"></div>
            </div>
    ...
    <div v-if="showDeviceDetails && deviceStore.selectedDevice" class="fixed inset-0 bg-black/80 backdrop-blur-xl z-[60] flex items-center justify-center p-6" @click.self="showDeviceDetails = false">
      <div class="w-full max-w-2xl bg-[#1C1C1E] border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div class="p-6 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
          ...
          <button @click="showDeviceDetails = false" class="p-2 rounded-full hover:bg-white/10 text-[#8E8E93] transition"><svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>
        <div class="flex-1 overflow-y-auto p-6 space-y-8">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="space-y-3">
              <div class="flex items-center justify-between"><h4 class="text-sm font-semibold text-white">{{ t.apps.details.storageTitle }}</h4><span class="text-xs text-[#8E8E93]">{{ formatAppSize(deviceStore.selectedDevice.storageFree) }} {{ t.apps.details.storageStatus }} {{ formatAppSize(deviceStore.selectedDevice.storageTotal) }}</span></div>
              <div class="h-2.5 w-full bg-white/5 rounded-full overflow-hidden border border-white/10"><div class="h-full bg-[#0A84FF] rounded-full" :style="{ width: `${deviceStore.selectedDevice.storageTotal > 0 ? ((deviceStore.selectedDevice.storageTotal - deviceStore.selectedDevice.storageFree) / deviceStore.selectedDevice.storageTotal) * 100 : 0}%` }"></div></div>
              <div class="flex justify-between text-[10px] text-[#8E8E93] font-medium px-1"><div class="flex items-center space-x-1.5"><span class="w-2 h-2 rounded-full bg-[#0A84FF]"></span><span>Used: {{ formatAppSize(deviceStore.selectedDevice.storageUsed) }}</span></div><div class="flex items-center space-x-1.5"><span class="w-2 h-2 rounded-full bg-white/10"></span><span>Free: {{ formatAppSize(deviceStore.selectedDevice.storageFree) }}</span></div></div>
            </div>
            <div class="space-y-3">
              <div class="flex items-center justify-between"><h4 class="text-sm font-semibold text-white">{{ t.apps.details.batteryTitle }}</h4><div class="flex items-center space-x-2"><span v-if="deviceStore.selectedDevice.batteryCharging" class="text-[10px] font-bold text-[#30D158] uppercase">{{ t.apps.details.batteryStatus }}</span><span class="text-sm font-bold text-white">{{ deviceStore.selectedDevice.batteryLevel }}%</span></div></div>
              <div class="h-2.5 w-full bg-white/5 rounded-full overflow-hidden border border-white/10 relative"><div class="h-full rounded-full transition-all duration-500" :class="deviceStore.selectedDevice.batteryLevel > 20 ? 'bg-[#30D158]' : 'bg-[#FF453A]'" :style="{ width: `${deviceStore.selectedDevice.batteryLevel}%` }"></div></div>
          ...
          <div class="p-4 rounded-2xl bg-white/[0.03] border border-white/5 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-y-4">
              ...
              <div class="space-y-1 text-right"><div class="text-[10px] font-bold text-[#8E8E93] uppercase">{{ t.apps.details.jailbreak }}</div><div class="text-xs font-bold" :class="deviceStore.selectedDevice.isJailbroken ? 'text-[#FF453A]' : 'text-[#30D158]'">{{ deviceStore.selectedDevice.isJailbroken ? 'Yes' : 'No' }}</div></div>
              <div class="space-y-1"><div class="text-[10px] font-bold text-[#8E8E93] uppercase">{{ t.apps.details.udid }}</div><div class="text-[10px] text-white font-mono break-all">{{ deviceStore.selectedDevice.udid }}</div></div>
            </div>
          </div>
        </div>
...
import type { InstalledApp, DeviceInstallTask } from '../types'
